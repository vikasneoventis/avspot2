...
function save_settings() {
	...
	jQuery("#save-settings-button i").attr('class', 'fa fa-spinner fa-spin');
	...
	jQuery.ajax({
		...
		success	: function(return_data) {
			...
			jQuery("#save-settings-button i").attr('class', 'fa fa-check');
			var data;
			try {
...
function plugin_uploaded() {
	...
	try {
		...
		var status = data.status;
		if (status == "OK") {
			//jQuery("#upload-message").html("<div class='global-message global-message-success'>"+data.message+"</div>");
			//jQuery("#upload-message").slideDown(250);
			location.href = data.url;
		} else if (status == "ERROR") {
			jQuery("#plugins-item-new i").attr("class", "fa fa-plus");
	...
	jQuery.ajax({
		...
		success	: function(return_data) {
			...
			try {
				...
				var status = data.status;
				if (status == "OK") {
					//jQuery("#toggle-plugin-message").html("<div class='global-message global-message-success'>"+data.message+"</div>");
...
function test_mailing() {
	...
	jQuery("#test-mailing-button i").attr('class', 'fa fa-spinner fa-spin');
	...
	jQuery.ajax({
		...
		success	: function(return_data) {
			...
			jQuery("#test-mailing-button i").attr('class', 'fa fa-envelope-o');
			var data;
			try {
				var data = jQuery.parseJSON(return_data);
				var status = data.status;
