...
function login() {
	jQuery("#login").html('<i class="fa fa-spinner fa-spin"></i> Login');
	jQuery("#message").slideUp(250);
	loading = true;
	var post_data = {};
	...
	jQuery.ajax({
		...
		success	: function(return_data) {
			loading = false;
			jQuery("#login").html('<i class="fa fa-angle-double-right"></i> Login');
			var data;
			try {
...
function reset_password() {
	jQuery("#reset").html('<i class="fa fa-spinner fa-spin"></i> Reset');
	jQuery("#message").slideUp(250);
	loading = true;
	var post_data = {};
	...
	jQuery.ajax({
		...
		success	: function(return_data) {
			loading = false;
			jQuery("#reset").html('<i class="fa fa-angle-double-right"></i> Reset');
			var data;
			try {
				var data = jQuery.parseJSON(return_data);
				var status = data.status;
