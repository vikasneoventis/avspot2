...
class rs_whiteboard_fe_slider {
	...
	public static function init(
		...
		$frameworkHelper->add_action('revslider_fe_javascript_option_output', array('rs_whiteboard_fe_slider', 'add_whiteboard_javascript_options'));
		$frameworkHelper->add_action('revslider_fe_javascript_output', array('rs_whiteboard_fe_slider', 'add_whiteboard_javascript'), 10, 2);
		//$frameworkHelper->add_action('revslider_preview_slider_head' ,array('rs_whiteboard_fe_slider', 'enqueue_scripts_html')); //add the JavaScript to the Preview
	}
	...
	public static function add_whiteboard_javascript($slider, $htmlid){
		...
		if($slider->getParam("wb_enable","off") === 'on' && $slider->getParam('wb_is_used', false) === true){
			echo "				require(['" . WHITEBOARD_PLUGIN_URL . "public/assets/js/revolution.addon.whiteboard.min.js'], function() {" . "\n";
	...
	public static function enqueue_scripts(){
		self::$frameworkHelper->wp_enqueue_script('rs-whiteboard', WHITEBOARD_PLUGIN_URL .'public/assets/js/revolution.addon.whiteboard.min.js', array('jquery','revmin'), WHITEBOARD_VERSION);
