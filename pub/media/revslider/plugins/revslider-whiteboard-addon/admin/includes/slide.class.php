...
class rs_whiteboard_slide {
	...
	public static function init(\Nwdthemes\Revslider\Helper\Framework $frameworkHelper) {

        self::$frameworkHelper = $frameworkHelper;
		
		self::$frameworkHelper->add_filter('revslider_slide_addons', array('rs_whiteboard_slide', 'add_whiteboard_settings'), 10, 3);
		self::$frameworkHelper->add_action('revslider_fe_javascript_option_output', array('rs_whiteboard_slide', 'add_fe_javascript_options'));
...
',
		   'javascript'=> '
			...
			var wb_writehand_sources = {
				handtype:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['type']).'",
				direction:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['direction']).'",
				src:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['src']).'",
				width:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['width']).'",
				height:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['height']).'",
				origin_x:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['origin_x']).'",
				origin_y:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['origin_y']).'",
				jitter:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['jitter']).'",
				jitter_horizontal:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['jitter_horizontal']).'",
				jitter_repeat:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['jitter_repeat']).'",
				jitter_offset:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['jitter_offset']).'",
				jitter_offset_horizontal:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['jitter_offset_horizontal']).'",
				angle:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['angle']).'",
				angle_repeat:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['angle_repeat']).'"
			};
			   
			var wb_movehand_sources = {
				handtype:"'.self::$frameworkHelper->esc_attr($handvals['movehand']['type']).'",
				src:"'.self::$frameworkHelper->esc_attr($handvals['movehand']['src']).'",
				width:"'.self::$frameworkHelper->esc_attr($handvals['movehand']['width']).'",
				height:"'.self::$frameworkHelper->esc_attr($handvals['movehand']['height']).'",
				origin_x:"'.self::$frameworkHelper->esc_attr($handvals['movehand']['origin_x']).'",
				origin_y:"'.self::$frameworkHelper->esc_attr($handvals['movehand']['origin_y']).'",
				jitter:"'.self::$frameworkHelper->esc_attr($handvals['movehand']['jitter']).'",
				jitter_horizontal:"'.self::$frameworkHelper->esc_attr($handvals['movehand']['jitter_horizontal']).'",
				jitter_repeat:"'.self::$frameworkHelper->esc_attr($handvals['movehand']['jitter_repeat']).'",
				jitter_offset:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['jitter_offset']).'",
				jitter_offset_horizontal:"'.self::$frameworkHelper->esc_attr($handvals['writehand']['jitter_offset_horizontal']).'",
				angle:"'.self::$frameworkHelper->esc_attr($handvals['movehand']['angle']).'",
				angle_repeat:"'.self::$frameworkHelper->esc_attr($handvals['movehand']['angle_repeat']).'"
			};'
			. "require(['admin', '" . WHITEBOARD_PLUGIN_URL . "admin/assets/js/whiteboard-admin.js'], function(UniteAdminRev) {"
			. "    UniteAdminRev.loadCssFile('" . WHITEBOARD_PLUGIN_URL . "admin/assets/css/whiteboard-admin.css', 'whiteboard-admin', true);"
			. "});"
