...
class rs_whiteboard_slider {
	...
	public static function init(
		...
		$frameworkHelper->add_filter('revslider_slider_addons', array('rs_whiteboard_slider', 'add_whiteboard_settings'), 10, 2);

		if(isset(Data::$_GET["page"]) && Data::$_GET["page"]=="revslider"){
			$frameworkHelper->add_action('admin_enqueue_scripts', array('rs_whiteboard_slider', 'wb_enqueue_styles'));
			$frameworkHelper->add_action('admin_enqueue_scripts', array('rs_whiteboard_slider', 'wb_enqueue_scripts'));
		}
	}
	
	public static function wb_enqueue_styles(){
		self::$frameworkHelper->wp_register_style('revslider-whiteboard-plugin-settings', WHITEBOARD_PLUGIN_URL . 'admin/assets/css/whiteboard-admin.css', array(), WHITEBOARD_VERSION);
		self::$frameworkHelper->wp_enqueue_style('revslider-whiteboard-plugin-settings');
	}
	
	
	public static function wb_enqueue_scripts(){
		self::$frameworkHelper->wp_register_script('revslider-whiteboard-plugin-js', WHITEBOARD_PLUGIN_URL . 'admin/assets/js/whiteboard-admin.js', array(), WHITEBOARD_VERSION);
		self::$frameworkHelper->wp_enqueue_script('revslider-whiteboard-plugin-js');
	}
...
<select id="wb_movehand_type" name="wb_movehand_type" class="withlabel">
	...
	<option value="left"';
		if($wb_writehand_direction == "rtl") $markup .= ' selected="selected"';
		$markup .= '>'.__("Right to Left", 'rs_whiteboard').'</option>
	<option value="top"';
		if($wb_writehand_direction == "btt") $markup .= ' selected="selected"';
		$markup .= '>'.__("Bottom to Top", 'rs_whiteboard').'</option>
	<option value="bottom"';
		if($wb_writehand_direction == "ttb") $markup .= ' selected="selected"';
