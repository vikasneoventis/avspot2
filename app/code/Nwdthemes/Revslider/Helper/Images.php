...
class Images extends \Magento\Framework\App\Helper\AbstractHelper {
	...
	public function image_downsize($id, $size = 'medium') {
		...
		if ((string)(int)$id === (string)$id && $product = $this->_productFactory->create()->load($id)) {
            ...
            // Other method to get images works in front end only TODO: take care of product image sizes
            /*$this->_imageBuilder->setProduct($product);
            switch ($size) {
                case 'thumbnail' :
                    $this->_imageBuilder->setImageId('product_thumbnail_image');
                break;
                case 'small' :
                    $this->_imageBuilder->setImageId('product_small_image');
                break;
                case 'medium' :
                    $this->_imageBuilder->setImageId('product_page_image_medium');
                break;
                case 'large' :
                    $this->_imageBuilder->setImageId('product_page_image_large');
                break;
                case 'base' :
                    $this->_imageBuilder->setImageId('product_base_image');
                break;
                case 'full' :
                default :
                    $this->_imageBuilder->setImageId('product_page_main_image');
                break;
            }
            $image = $this->_imageBuilder->create();
            $downsizedImage = [
                $image->getImageUrl(),
                $image->getWidth(),
                $image->getHeight()
