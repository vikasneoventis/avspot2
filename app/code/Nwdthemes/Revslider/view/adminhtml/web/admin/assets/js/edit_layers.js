	...
	*****************************************************/
	var initHtmlFields = function(){
		...
		jQuery('body').on('click','.timer-manual-edit, #layers-right .sortablelayers',function() {
            var li, index;
            if (jQuery(this).parents('div.sortlist').length) {
                li = jQuery(this).parents('li');
                index = jQuery('#layers-left > ul > li').index(li);
            } else {
                li = jQuery(this);
                index = jQuery('#layers-right > ul > li').index(li);
            }
			jQuery('#timline-manual-dialog').css({top: (li.outerHeight(true) * index - 80) + 'px'}).show();
        });
