...
class RevSliderOperations extends \Nwdthemes\Revslider\Model\Revslider\Framework\RevSliderElementsBase {
	...
	public function getArrFontFamilys($slider = false){
        ...
        return $this->_framework->apply_filters('revslider_operations_getArrFontFamilys', $fonts);
	}
	...
	public static function getStaticCss(){
		if ( self::$framework->is_multisite() ){
	...
	public function renameCaption($content){
		if(isset($content['old_name']) && isset($content['new_name'])) {
			...
			if($result !== false){ //rename all layers in all Sliders that use this old name with the new name
				$slider = new RevSliderSlider($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
	...
	public function previewOutput($sliderID,$output = null, $storeID = 0) {
		...
		if($output == null)
			$output = new RevSliderOutput($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
...
ob_end_clean();
		...
		if($export_real){ //only include if real export
			...
			if(!$usepcl){
				...
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/jquery.themepunch.enablelog.js', 'js/jquery.themepunch.enablelog.js');
				...
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.css', 'fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.css');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/pe-icon-7-stroke/css/helper.css', 'fonts/pe-icon-7-stroke/css/helper.css');
				...
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/jquery.themepunch.enablelog.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
				...
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.css', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/pe-icon-7-stroke/css/helper.css', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
	...
	public function putSlidePreviewByData($data){
		...
		$slide = new RevSliderSlide($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
	...
	public function checkPurchaseVerification($data){
		...
		$response = $this->_framework->wp_remote_post('http://updates.themepunch.tools/activate.php', array(
		...
		if($version_info == 'valid'){
            ...
            //$this->_framework->update_option('revslider-email', $data['email']);
	...
	public function doPurchaseDeactivation($data){
		...
		$response = $this->_framework->wp_remote_post('http://updates.themepunch.tools/deactivate.php', array(
			'user-agent' => 'Magento/'.$wp_version.'; '.$this->_framework->get_bloginfo('url'),
			'body' => array(
				'code' => urlencode($code),
				'product' => urlencode('revslider_magento')
	...
	public static function get_slider_speed($sliderID){
		//$data = self::$images->wp_get_attachment_metadata($cur_img_id);
