...
class RevSliderSlider extends \Nwdthemes\Revslider\Model\Revslider\Framework\RevSliderElementsBase {
    ...
    private function duplicateSlider($title = false, $prefix = false){
		...
		if(!empty($slides)){
			foreach($slides as $slide){
				$c_slide = new RevSliderSlide($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
		...
		$c_slider = new RevSliderSlider($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
	...
	private function copyMoveSlide($slideID,$targetSliderID,$operation){
		...
		if($operation == "move"){
			...
			$targetSlider = new RevSliderSlider($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
		...
		try{
            ...
            if( !$this->_framework->is_wp_error($unzipfile) ){
				...
				if($is_template !== false){
					...
					if($uid_check !== ''){
						$tmpl = new RevSliderTemplate($this->_framework, $this->_query, $this->_curl, $this->_filesystem);
				...
				if(!empty($animations)){
					...
					$this->_framework->dmp(__("animations imported!"));
				}
				...
				if(is_array($dynamicCss) && $dynamicCss !== false && count($dynamicCss) > 0){
					...
					$this->_framework->dmp(__("dynamic styles imported!"));
				}
                ...
                if(!empty($navigations)){
                    ...
                    $this->_framework->dmp(__("navigations imported!"));
				}
            }else{
			...
			if($is_template !== false){ //duplicate the slider now, as we just imported the "template"
				if($single_slide !== false){ //add now one Slide to the current Slider
					$mslider = new RevSliderSlider($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
	...
	private function duplicateChildren($slideID,$newSlideID){
		...
		foreach($arrChildren as $childSlide){
			...
			//update parent id
			$duplicatedSlide = new RevSliderSlide($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
	...
	public function getUsedFonts($full = false){
        ...
        $sl = new RevSliderSlide($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
        
		$mslides = $this->getSlides(true);
        
        $staticID = $sl->getStaticSlideID($this->getID());
        if($staticID !== false){
            $msl = new RevSliderSlide($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
