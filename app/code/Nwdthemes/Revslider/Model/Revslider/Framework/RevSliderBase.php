...
class RevSliderBase {
	...
	public static function getImageUrl($filepath, $width=null,$height=null,$exact=false,$effect=null,$effect_param=null){
		...
		self::$framework->log('getImageUrl', $filepath, $width, $height);
		self::$framework->log('self::$url_ajax_showimage', self::$url_ajax_showimage);

        $urlImage = self::getUrlThumb(self::$url_ajax_showimage, $filepath,$width ,$height ,$exact ,$effect ,$effect_param);

		self::$framework->log('return', $urlImage);
	...
	public static function onShowImage(){
		...
		try{
			...
			$img = self::$images->wp_get_attachment_image_src( $imageID, 'thumb' );
	...
	*/
	public static function get_all_image_sizes($type = 'gallery'){
		...
		switch($type){
			...
			case 'gallery':
			default:
				$added_image_sizes = self::$images->get_intermediate_image_sizes();
