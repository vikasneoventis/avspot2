...
class RevSliderFunctionsWP {
	...
	public static function import_media($file_url, $folder_name) {
		//require_once(Framework::ABSPATH . 'wp-admin/includes/image.php');
		...
		//$siteurl = self::$framework->get_option('siteurl');
		...
		if (strpos($filename, '.') !== false && @fclose(@fopen($file_url, "r"))) {
			...
			if($atc_id == false || $atc_id == NULL){
				...
				$attach_id = self::$images->wp_insert_attachment($artdata, $artDir.$folder_name.$filename);
			}else{
				$attach_id = $atc_id;
			}

			//generate metadata and thumbnails
			if($attach_data = self::$images->wp_generate_attachment_metadata($attach_id, $saveDir)) self::$images->wp_update_attachment_metadata($attach_id, $attach_data);
	...
	public static function getUserDisplayName($userID){
		...
		$displayName =  self::$framework->get_the_author_meta('display_name', $userID);
