...
class RevSliderFunctions {
	...
	public static function validateFilepath($filepath,$errorPrefix=null){
		...
		if($errorPrefix == null)
			$errorPrefix = "File";
		$message = $errorPrefix." ".$this->_framework->esc_attr($filepath)." not exists!";
	...
	public static function get_responsive_size($slider){
		...
		if($adv_resp_sizes == true){
			...
			$height .= ','. $slider->slider->getParam("height_notebook", 768, RevSliderSlider::FORCE_NUMERIC);
			$height .= ','. intval($slider->slider->getParam("height_tablet", 960, RevSliderSlider::FORCE_NUMERIC));
			$height .= ','. intval($slider->slider->getParam("height_mobile", 720, RevSliderSlider::FORCE_NUMERIC));
