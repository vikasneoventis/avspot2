...
class RevSliderTinyBox {
    ...
    public static function add_tinymce_editor(){
        ...
        $post_types = $this->_framework->get_post_types();
        ...
        $this->_framework->add_action('in_admin_footer', array('RevSliderTinyBox', 'add_tiny_mce_shortcode_dialog'));
    ...
    public static function visual_composer_include(){
        ...
        if($this->_framework->is_user_logged_in()){

            if(!function_exists('vc_map') || !function_exists('vc_action')) return false;

            if('vc_inline' === vc_action() || $this->_framework->is_admin()){
                $this->_framework->add_action( 'init', array('RevSliderTinyBox', 'add_to_VC' ));
            }
        }
	}
    ...
    public static function add_to_VC() {
        ...
        //$this->_framework->wp_enqueue_script('revslider-admin-script', Framework::$RS_PLUGIN_URL.'admin/assets/js/admin.js', array('jquery'), RevSliderGlobals::SLIDER_REVISION );
        $this->_framework->wp_enqueue_script('wpdialogs', 'jquery-ui-sortable', 'jquery-ui-dialog');
        $this->_framework->wp_enqueue_style('wp-jquery-ui-dialog');
        ...
        vc_map( array(
            ...
            'admin_enqueue_js' => Framework::$RS_PLUGIN_URL.'admin/assets/js/vc.js',
            'front_enqueue_js' => Framework::$RS_PLUGIN_URL.'admin/assets/js/vc.js',
            'params' => array(
                array(
    ...
    public static function add_tinymce_shortcode_editor_plugin($plugin_array){
        ...
        $plugin_array['revslider_sc_button'] = Framework::$RS_PLUGIN_URL . 'admin/assets/js/tinymce-shortcode-script.js';
    ...
    public static function add_tiny_mce_shortcode_dialog(){
        $sld = new RevSliderSlider(self::$framework, self::$query, self::$curl, self::$filesystem, self::$images, self::$resource, self::$googleFonts);
        ...
        <div id="revslider-tiny-mce-dialog" tabindex="-1" action="" title="" style="display: none; ">
            <form id="revslider-tiny-mce-settings-form" action="">
                ...
                <div id="revslider-tiny-dialog-step-1">
                    ...
                    <div class="revslider-quick-inner-wrapper" style="padding-right:0px;padding-bottom:0px;">
                        <select name="revslider-existing-slider" id="revslider-existing-slider">
                            <option value="-1" selected="selected"><?php echo __('--- Choose Slider ---'); ?></option>
                            <?php
                            $sl = array();
                            $sliders_info = array();
                            if(!empty($sliders)){
                                    ...
                                    if(!empty($slides)){
                                        ...
                                        foreach($slides as $slide){
                                            ...
                                            if($bgt == 'image' || $bgt == 'streamvimeo' || $bgt == 'streamyoutube' || $bgt == 'streaminstagram'){
                                                switch($type){
                                                    case 'posts':
                                                        $urlImageForView = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/post.png';
                                                    break;
                                                    case 'woocommerce':
                                                        $urlImageForView = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/wc.png';
                                                    break;
                                                    case 'facebook':
                                                        $urlImageForView = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/fb.png';
                                                    break;
                                                    case 'twitter':
                                                        $urlImageForView = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/tw.png';
                                                    break;
                                                    case 'instagram':
                                                        $urlImageForView = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/ig.png';
                                                    break;
                                                    case 'flickr':
                                                        $urlImageForView = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/fr.png';
                                                    break;
                                                    case 'youtube':
                                                        $urlImageForView = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/yt.png';
                                                    break;
                                                    case 'vimeo':
                                                        $urlImageForView = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/vm.png';
                                                    break;
                                                }
                                            }
                                ...
                                if(!empty($sl)){
                                    foreach($sl as $type => $slider){
                                        ...
                                        echo '<option disabled="disabled">--- '.ucfirst($this->_framework->esc_attr($mtype)).' ---</option>';
                                        foreach($slider as $values){
                                            if($values['alias'] != 'false'){
                                                echo '<option data-sliderid="'.$this->_framework->esc_attr($values['id']).'" data-slidertype="'.$this->_framework->esc_attr($type).'" value="'.$this->_framework->esc_attr($values['alias']).'">'.$this->_framework->esc_attr($values['title']).'</option>'."\n";
                                            }
                                        }
                                    }
                                }
                        ...
                        <ul id="rs-shortcode-select-wrapper">
                            <li class="rs-slider-modify-li rs-slider-modify-new-slider">
                                <a href="<?php echo RevSliderBaseAdmin::getViewUrl(RevSliderAdmin::VIEW_SLIDER); ?>" target="_blank" class="button-primary revgreen" id="rs-create-predefined-slider"><span class="dashicons dashicons-plus"></span><span class="rs-slider-modify-title"><?php echo __('Create Slider'); ?></span></a>
                            </li>
                            <?php
                            if(!empty($sliders_info)){
                                foreach($sliders_info as $type => $slider){
                                    foreach($slider as $values){
                                        ?>
                                        <li id="slider_list_item_<?php echo $values['sliderid']; ?>" class="rs-slider-modify-li" data-sliderid="<?php echo $this->_framework->esc_attr($values['sliderid']); ?>" data-slideralias="<?php echo $this->_framework->esc_attr($values['slideralias']); ?>">
                    ...
                    <div class="revslider-stepnavigator">
                        <span class="revslider-currentstep"><strong><?php echo __('STEP 1'); ?></strong><?php echo __('Select / Create Slider'); ?></span>
                        <span class="revslider-step-actions-wrapper">
                            <a href="javascript:void(0);" class="button-primary nonclickable" id="rs-modify-predefined-slider"><span class="dashicons dashicons-admin-generic"></span><?php echo __('Quick Modify Slider'); ?></a>
                            <a href="javascript:void(0);" class="button-primary nonclickable" id="rs-add-predefined-slider"><span class="dashicons dashicons-migrate"></span><?php echo __('Add Selected Slider'); ?></a>
                        </span>
                        <span style="clear:both;width:100%;display:block"></span>
                    </div>
                </div>
                ...
                <div id="revslider-tiny-dialog-step-1-5" style="display: none;">
                    ...
                    <div class="revslider-stepnavigator"><span class="revslider-currentstep"><strong><?php echo __('STEP 2'); ?></strong><?php echo __('Quick Modify / Add Slider'); ?></span>
                        <span class="revslider-step-actions-wrapper">
                            <a href="javascript:void(0);" class="button-primary  rs-goto-step-1"><span class="dashicons dashicons-arrow-left-alt"></span><?php echo __('Select Slider'); ?></a>
                            <a href="javascript:void(0);" class="button-primary " id="revslider-add-custom-shortcode-modify"><span class="dashicons dashicons-migrate"></span><?php echo __('Add Selected Slider'); ?></a>
                        </span>
                    </div>
                </div>
            </form>
