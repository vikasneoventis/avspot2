...
class RevSliderOutput {
    ...
    public static function putSlider($sliderID,$putIn="", $gal_ids = array(), $settings = array(), $order = array()){
		...
		$output = new RevSliderOutput(self::$framework, self::$query, self::$curl, self::$filesystem, self::$images, self::$resource, self::$googleFonts);
	...
	private function filterOneSlide($slides){
        ...
        if(strpos($oneSlideID, 'static_') !== false){
            ...
            if(!empty($tmp_slides)){
                $n_slides = new RevSliderSlide($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
    ...
    private function putSlides($gal_ids = array(), $order = array()){
		...
		if($this->previewMode == true && $this->oneSlideMode == true){
			...
			$previewSlide = new RevSliderSlide($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
        ...
        if($do_static){
            ...
            foreach($slides as $slide){
                ...
                if($staticID !== false){
                    $static_slide = new RevSliderSlide($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
		...
		foreach($slides as $slide){
			...
			if($isExternal != 'external'){
				...
				if($img_id !== false){
					$img_data = $this->_images->wp_get_attachment_metadata( $img_id );
		...
		foreach($layers as $layer){
			...
			**/
			...
			switch($type){
				...
				case 'button':
					$html = $text;
					$html = $this->_framework->do_shortcode(stripslashes($html), $slide);
                    $html_toggle = $text_toggle;
                    $html_toggle = $this->_framework->do_shortcode(stripslashes($html_toggle), $slide);
					...
					foreach($icon_sets as $is){
						if(strpos($html, $is) !== false){ //include default Icon Sets if used
							...
							$$font_var = true;
							//$this->_framework->wp_enqueue_style('rs-icon-set-'.$is);
						}
					}
				...
				case 'image':
					...
					if($img_size !== 'full'){
						if($cur_img_id !== false){
							$urlImage = $this->_images->wp_get_attachment_image_src($cur_img_id, $img_size);
                            $urlImage = $urlImage[0];
						}
					}
                    ...
                    if (isset($urlImageData['1'])) {
                        $urlImage = $this->_images->getBaseUrl() . $urlImageData['1'];
                    }

					$img_w = '';
					$img_h = '';
					if($cur_img_id !== false){
						$img_data = $this->_images->wp_get_attachment_metadata( $cur_img_id );
						if($img_data !== false && !empty($img_data)){
							if($img_size !== 'full'){
								if(isset($img_data['sizes']) && isset($img_data['sizes'][$img_size])){
			...
			if($animation_overwrite == 'waitout' || $animation_overwrite == 'wait'){
				...
				if($realEndTime !== false){
					$endSpeed = trim(RevSliderFunctions::getVal($layer, "endspeed"));
					$speed = RevSliderFunctions::getVal($layer, "speed",300);
				...
				foreach($a_action as $num => $action){
					...
					switch($action){
						...
						case 'link':
							...
							//a_image_link
                            if(isset($a_image_link[$num])) $a_image_link[$num] = $this->_framework->do_shortcode($a_image_link[$num], $slide);
            ...
            $this->_framework->do_action('revslider_add_layer_attributes', $layer, $slide, $this->slider);
			echo $a_html;
            ...
            echo $this->_framework->apply_filters('revslider_layer_content', stripslashes($html), $html, $this->slider->getID(), $slide, $layer)." ";
            if ($toggle_allow!=false) {
                echo '</div>';
...
<?php if($this->slider->getParam("slider_type") == "fullscreen"){ ?>
					e.fullScreenAutoWidth='<?php echo $this->_framework->esc_attr($this->slider->getParam("autowidth_force","off")); ?>';
					e.fullScreenAlignForce='<?php echo $this->_framework->esc_attr($this->slider->getParam("full_screen_align_force","off")); ?>';
					e.fullScreenOffsetContainer= '<?php echo $this->_framework->esc_attr($this->slider->getParam("fullscreen_offset_container","")); ?>';
					e.fullScreenOffset='<?php echo $this->_framework->esc_attr($this->slider->getParam("fullscreen_offset_size","")); ?>';
<?php } ?>
...
<?php } ?>
            ...
            if($slider_type !== 'hero' && ($enable_arrows == 'on' || $enable_bullets == 'on' || $enable_tabs == 'on' || $enable_thumbnails == 'on' || $touch_enabled == 'on' || $keyboard_enabled == 'on' || $mousescroll_enabled != 'off')){
				...
				if($enable_thumbnails == 'on'){
					...
					echo '								wrapper_color:"'. $this->_framework->esc_attr($this->slider->getParam('thumbnails_wrapper_color','transparent')).'",'."\n";
					echo '								wrapper_opacity:"'. $this->_framework->esc_attr(round($this->slider->getParam('thumbnails_wrapper_opacity','100') / 100, 2)).'",'."\n";
					echo '								tmp:\'';
					echo preg_replace( "/\r|\n/", "", $thumbs_tmp);
					echo '\','."\n";
				...
				if($enable_tabs == 'on'){
					...
					echo '								wrapper_color:"'. $this->_framework->esc_attr($this->slider->getParam('tabs_wrapper_color','transparent')).'",'."\n";
					echo '								wrapper_opacity:"'. $this->_framework->esc_attr(round($this->slider->getParam('tabs_wrapper_opacity','100') / 100, 2)).'",'."\n";
					echo '								tmp:\'';
					echo preg_replace( "/\r|\n/", "", $tabs_tmp);
					echo '\','."\n";
			...
			if($use_parallax == 'on'){
				...
				if ($parallax_type == '3D') {
					...
					//echo '							ddd_path:"'. $this->_framework->esc_attr($parallax_ddd_path) .'",'."\n";
				}				
        ...
        if(!$markup_export){ //not needed for html markup export
            $this->_framework->add_action('wp_footer', array($this, 'add_inline_double_jquery_error'));
        }
	}
    ...
    public function putSliderBase($sliderID, $gal_ids = array(), $markup_export = false, $settings = array(), $order = array()){
		...
		try{
			...
			$this->slider = new RevSliderSlider($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
			if($sliderID !== '-99'){
			...
			if($this->slider->getParam("js_to_body","false") == "true"){
				...
				if($enable_logs == 'on'){
					$urlIncludeJS = Framework::$RS_PLUGIN_URL.'public/assets/js/jquery.themepunch.enablelog.js?rev='. $slver;
					$htmlBeforeSlider .= '<script type="text/javascript" src="'.$urlIncludeJS.'"></script>';
				}

				$urlIncludeJS = Framework::$RS_PLUGIN_URL.'public/assets/js/jquery.themepunch.tools.min.js?rev='. $slver;
				$htmlBeforeSlider .= '<script type="text/javascript" src="'.$urlIncludeJS.'"></script>';
				$urlIncludeJS = Framework::$RS_PLUGIN_URL.'public/assets/js/jquery.themepunch.revolution.min.js?rev='. $slver;
				$htmlBeforeSlider .= '<script type="text/javascript" src="'.$urlIncludeJS.'"></script>';
			...
			$progress_style = ' style="height: '.$this->_framework->esc_attr($progress_height).'px; background-color: '.RevSliderFunctions::hex2rgba($progressbar_color, $progress_opa).';"';
