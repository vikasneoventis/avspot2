...
class RevSliderAdmin extends \Nwdthemes\Revslider\Model\Revslider\Framework\RevSliderBaseAdmin {
	...
	private function init(){
		...
		$template = new RevSliderTemplate($this->_framework, $this->_query, $this->_curl, $this->_filesystem);
		$operations = new RevSliderOperations($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts);
        ...
        if(isset($pagenow) && $pagenow == 'plugins.php'){
            $this->_framework->add_action('admin_notices', array('RevSliderAdmin', 'add_plugins_page_notices'));
        }

        // Add-on Admin
        $addon_admin = new RevSliderAddonAdmin('rev_addon', RevSliderGlobals::SLIDER_REVISION, $this->_filesystem, $this->_framework, $this->_plugin);
        ...
        $this->_framework->add_action( 'wp_ajax_nopriv_activate_plugin', array( $addon_admin, 'activate_plugin') );
        $this->_framework->add_action( 'wp_ajax_deactivate_plugin', array( $addon_admin, 'deactivate_plugin'));
        $this->_framework->add_action( 'wp_ajax_nopriv_deactivate_plugin', array( $addon_admin, 'deactivate_plugin') );
        $this->_framework->add_action( 'wp_ajax_install_plugin', array( $addon_admin, 'install_plugin'));
        $this->_framework->add_action( 'wp_ajax_nopriv_install_plugin', array( $addon_admin, 'install_plugin') );
    ...
    public static function plugin_action_links($links, $file){
        if ($file == self::$plugin->plugin_basename(RS_PLUGIN_FILE_PATH)){
	...
	public static function enqueue_styles(){
        ...
        self::$framework->wp_enqueue_style('revslider-global-styles', Framework::$RS_PLUGIN_URL . 'admin/assets/css/global.css', array(), RevSliderGlobals::SLIDER_REVISION );
	}
	...
	public static function include_custom_css(){
		...
		switch($type){
			...
			case 'slide':
				...
				$slide = new RevSliderSlide(self::$framework, self::$query, self::$curl, self::$filesystem, self::$images, self::$resource, self::$googleFonts);
		...
		if(!empty($sliderID)){
			$slider = new RevSliderSlider(self::$framework, self::$query, self::$curl, self::$filesystem, self::$images, self::$resource, self::$googleFonts);
	...
	public static function enqueue_all_admin_scripts() {
        ...
        $screen = self::$framework->get_current_screen();

        $post_types = self::$framework->get_post_types( '', 'names' );
        foreach($post_types as $post_type) {
            if($post_type == $screen->id){
                self::$framework->wp_enqueue_script('revslider-tinymce-shortcode-script', Framework::$RS_PLUGIN_URL . 'admin/assets/js/tinymce-shortcode-script.js', array('jquery'), RevSliderGlobals::SLIDER_REVISION );
            }
        }
	}
			...
			if(!empty($notices) && is_array($notices)){
				...
				foreach($notices as $notice){
                    ...
                    if(!in_array($notice->code, $notices_discarded) && version_compare($notice->version, RevSliderGlobals::SLIDER_REVISION, '>=')){
                        $text = '<div style="text-align:right;vertical-align:middle;display:table-cell; min-width:225px;border-left:1px solid #ddd; padding-left:15px;"><a href="javascript:void(0);"  class="rs-notices-button rs-notice-'. $this->_framework->esc_attr($notice->code) .'">'. __('Close & don\'t show again<b>X</b>','revslider') .'</a></div>';
	...
	public static function customPostFieldsOutput(){
		...
		$meta = $this->_framework->get_post_meta(get_the_ID(), 'slide_template', true);
				...
				<div class="setting_input">
					<select name="slide_template" id="slide_template">
						...
						foreach($arrOutput as $handle => $name){
							echo '<option '.$this->_framework->selected($handle, $meta).' value="'.$handle.'">'.$name.'</option>';
						}
						?>
					</select>
				</div>
	...
	public static function onAddScripts(){
		...
		self::$framework->wp_enqueue_style('edit_layers', Framework::$RS_PLUGIN_URL .'admin/assets/css/edit_layers.css', array(), RevSliderGlobals::SLIDER_REVISION);
		...
		//add icon sets
		self::$framework->wp_enqueue_style('rs-icon-set-fa-icon-', Framework::$RS_PLUGIN_URL .'public/assets/fonts/font-awesome/css/font-awesome.css', array(), RevSliderGlobals::SLIDER_REVISION);
		self::$framework->wp_enqueue_style('rs-icon-set-pe-7s-', Framework::$RS_PLUGIN_URL .'public/assets/fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.css', array(), RevSliderGlobals::SLIDER_REVISION);
    ...
    private static function importSliderHandle($viewBack = null, $updateAnim = true, $updateStatic = true, $updateNavigation = true){
		...
		self::$framework->dmp(__("importing slider settings and data..."));
		...
		if($response["success"] == false){
			...
			self::$framework->dmp("<b>Error: ".$message."</b>");
			echo RevSliderFunctions::getHtmlLink($viewBack, __("Go Back"));
		} else{	//handle success, js redirect.
			self::$framework->dmp(__("Slider Import Success, redirecting..."));
	...
	private static function importSliderOnlineTemplateHandle($viewBack = null, $updateAnim = true, $updateStatic = true, $single_slide = false){
		self::$framework->dmp(__("downloading template slider from server..."));
		
		$uid = self::$framework->esc_attr(RevSliderFunctions::getPostVariable('uid'));

		if($uid == ''){
			self::$framework->dmp(__("ID missing, something went wrong. Please try again!"));
			echo RevSliderFunctions::getHtmlLink($viewBack, __("Go Back"));
			exit;
		}else{
			$tmp = new RevSliderTemplate(self::$framework, self::$query, self::$curl, self::$filesystem);
			
            $package = self::$framework->esc_attr(RevSliderFunctions::getPostVariable('package'));
            ...
            if(!empty($uids)){
                foreach($uids as $uid){
                    ...
                    if(is_array($filepath) && isset($filepath['error'])){
                        self::$framework->dmp($filepath['error']);
                    ...
                    if($filepath !== false){
                        ...
                        foreach($tmp_slider as $tslider){
                            if(isset($tslider['uid']) && $uid == $tslider['uid']){
                                if(!isset($tslider['installed'])){ //slider is installed
                                    ...
                                    $mSlider = new RevSliderSlider(self::$framework, self::$query, self::$curl, self::$filesystem, self::$images, self::$resource, self::$googleFonts);
                        ...
                        if($response["success"] == false){
                            ...
                            if(count($uids) > 1){
                                self::$framework->dmp(__("Slider Import Success", 'revslider'));
                            }else{
                                self::$framework->dmp(__("Slider Import Success, redirecting...",'revslider'));
                            }
                        }
                        ...
                        if(is_array($filepath)){
                            ...
                            self::$framework->dmp(__("Could not download from server. Please try again later!", 'revslider'));
                        }
                        echo RevSliderFunctions::getHtmlLink($viewBack, __("Go Back",'revslider'));
                        exit;
                    }
                ...
                self::$framework->dmp(__("Could not download package. Please try again later!", 'revslider'));
				exit;
			}
		}
	...
	private static function importSliderTemplateHandle($viewBack = null, $updateAnim = true, $updateStatic = true, $single_slide = false){
		...
		self::$framework->dmp(__("importing template slider settings and data..."));
		...
		try{
			...
			switch($action){
				...
				case "add_slide_to_template":
					$template = new RevSliderTemplate(self::$framework, self::$query, self::$curl, self::$filesystem);
                ...
                case "get_import_slides_data":
                    ...
                    if(!is_array($data)){
                        ...
                        if($staticID !== false){
                            $msl = new RevSliderSlide(self::$framework, self::$query, self::$curl, self::$filesystem, self::$images, self::$resource, self::$googleFonts);
				...
				case 'rs_get_store_information': 
					...
					$request = self::$framework->wp_remote_post('http://templates.themepunch.tools/revslider/store.php', array(
						'user-agent' => 'Magento/'.$wp_version.'; '.self::$framework->get_bloginfo('url'),
						'body' => $rattr
					));
		...
		if($isVerified){
			...
			switch(self::getPostVar('client_action', false)){
				case 'get_slider_html':
					...
					if($id > 0){
						$html = '';
						self::$framework->add_filter('revslider_add_js_delay', array('RevSliderAdmin', 'rev_set_js_delay'));
						ob_start();
